...
pub fn command() -> clap::Command {
    clap::Command::new("reclaim-deposit")
        .about("Remove an public attestation from the blockchain authorized by deposit owner")
        .arg(
            clap::Arg::new("id")
                .long("id")
                .required(false) // will be read from stdin if not provided
                .help("ID of the credential to remove"),
        )
}
...
pub async fn run(matches: &clap::ArgMatches) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let tx = crate::kilt::tx().public_credentials().reclaim_deposit(id);
