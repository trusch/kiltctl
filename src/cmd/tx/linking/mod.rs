...
pub async fn run(matches: &clap::ArgMatches) -> Result<(), Box<dyn std::error::Error>> {
    match matches.subcommand() {
        ...
        Some(("remove-sender-association", matches)) => {
            remove_sender_association::run(matches).await
        }
        Some(("associate-sender", matches)) => associate_sender::run(matches).await,
        Some(("remove-account-association", matches)) => {
            remove_account_association::run(matches).await
        }
